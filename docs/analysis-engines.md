...
| --------------- | ------------------------ | ------------------------ | ------------------------------------ |
| Code Quality    | `skills/code.md`         | 25%                      | AST metrics, SonarQube, custom rules |
| Architecture    | `skills/architecture.md` | 20%                      | Dependency graph analysis            |
| Security        | `skills/security.md`     | 30%                      | SonarQube, Semgrep, regex patterns   |
...
| SonarQube       | `skills/sonarqube.md`    | Augments code + security | SonarQube API, sonar-scanner         |
...
The SonarQube engine is an **optional augmentation layer** that runs after the Code Quality engine and before Architecture. It requires a running SonarQube server and the `sonar-scanner` CLI. When available, its metrics take precedence over grep-based estimates.

**What it measures:**

| Metric            | Source                                                           |
| ----------------- | ---------------------------------------------------------------- |
| Bugs              | AST-level reliability issues                                     |
| Vulnerabilities   | OWASP/CWE-mapped security flaws                                  |
| Security Hotspots | Code requiring human security review                             |
| Code Smells       | Maintainability issues (cognitive complexity, duplication, etc.) |
| Coverage          | Actual line/branch coverage (if test report provided)            |
| Technical Debt    | Estimated remediation effort                                     |
| Quality Gate      | Pass/fail verdict against configured thresholds                  |

**Availability:**

SonarQube is **mandatory** — analysis will not proceed without it. The engine verifies two prerequisites at startup:

1. Is `sonar-scanner` on `$PATH`?
2. Does `http://localhost:9000/api/system/status` return `{"status":"UP",...}`?

If either check fails, the analysis **halts** and the user is directed to `sonarqube/setup.md` to install and start SonarQube before retrying.

**Score adjustments to Code Quality and Security engines:**

| Condition           | Effect                                 |
| ------------------- | -------------------------------------- |
| Quality Gate PASSED | +1 to code score, +1 to security score |
| Quality Gate FAILED | -1 to code score, -1 to security score |
| 0 bugs              | +1 to code score                       |
| 0 vulnerabilities   | +1 to security score                   |
| Security Rating A   | +1 to security score                   |
| Security Rating D/E | -1 to security score                   |
| Coverage ≥ 30%      | Overrides test-ratio estimate          |
| Duplication < 3%    | +0.5 to code score                     |

**Supported languages (Community Edition):** JavaScript, TypeScript, Python, Java, C#, Go, PHP, Ruby.

**Setup:** See `sonarqube/setup.md` for Docker and CLI installation instructions.

**Configuration template:** `sonarqube/sonar-project.properties.template`
